...
void TransferFunctionDisplay::setSignalGraph(SignalGraph* graph) {
...
void TransferFunctionDisplay::setTargetNodeId(const std::string& nodeId) {
...
std::vector<std::pair<std::string, std::string>> TransferFunctionDisplay::getAvailableNodes() const {
    return cachedFilterNodes;
}

void TransferFunctionDisplay::updateTargetFilter() {
...
void TransferFunctionDisplay::mouseDown(const juce::MouseEvent& event) {
    if (cachedFilterNodes.size() > 1) {
...
void TransferFunctionDisplay::drawFilterSelector(juce::Graphics& g, juce::Rectangle<float> bounds) {
